...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.creacionkert.com/coleccion/maletas/${nameTitle}`,
    },
    openGraph: {
      title: maleta ? `${maleta.name} | Kert S.A.S - Maletas al Por Mayor` : 'Maleta no encontrada',
      ...
      url: `https://www.creacionkert.com/coleccion/maletas/${nameTitle}`,
      type: "website",
      locale: "es_CO",
      siteName: "Kert S.A.S",
      images: maleta?.images?.[0] ? [{
        url: `https://www.creacionkert.com${maleta.images[0]}`,
        width: 800,
        height: 600,
        alt: maleta.name,
      }] : [],
    ...
    twitter: {
      ...
      title: maleta ? `${maleta.name} | Kert S.A.S - Maletas al Por Mayor` : 'Maleta no encontrada',
      description: maleta ? `Detalles de ${maleta.name} - ${maleta.category}. Especificaciones, características y opciones de pedido al por mayor.` : 'Maleta no encontrada',
      images: maleta?.images?.[0] ? [`https://www.creacionkert.com${maleta.images[0]}`] : [],
    },
  };
}
...
export default async function MaletaPage({ params }) {
  ...
  const productImage = maleta.images?.[0]
    ? `https://www.creacionkert.com${maleta.images[0]}`
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "itemListElement": [
              ...
              { "@type": "ListItem", "position": 4, "name": maleta.name, "item": `https://www.creacionkert.com/coleccion/maletas/${nameTitle}` },
            ],
          }),
        }}
      />
